...
/**
 * posso definirla usando le stesse euristiche usante anche per
 * @see agente.Genetic_Algoritm.fitness.Fitness
 * */
public class funzione_utilità {
    ...
    public double Totale_FU(Stato s,double Delta,double T){
        this.Delta = Delta;
        this.T = T;
        return (0.55 * ElisirFitness(s) + (0.45 * CoverageFitness(s)));
    }
    public double ElisirFitness(Stato stato){
        List<Card> lista = stato.getCards();
        double avg = 0.0;
        int low = 0;
        int high = 0;
        for (Card card : lista) {
    ...
    public double CoverageFitness(Stato stato){
        ...
        for (Card card : cards) {
            ...
            if (card instanceof Troop){
                ...
                if(troop.getAttackScope() == Card.AttackScope.AIR_GROUND){
                    nAir++;
                }
            ...
            if (card instanceof DefensiveBuilding defensiveBuilding){
                if(defensiveBuilding.getAttackScope() == Card.AttackScope.AIR_GROUND){
                    nAir++;
                }
            ...
            if (card instanceof SpawnerTroop){
                ...
                List<Card> allCards = cardService.loadCards("src/main/java/cardList.json");
                ...
                if (spawnerTroop.getAttackScope() == Card.AttackScope.AIR_GROUND){
                    nAir++;
                }
            ...
            if (card instanceof SpawnerBuilding){
                ...
                Troop spawnedTroop = (Troop) allCards.stream()
                        ...
                        .findFirst()                        // Prendi il primo risultato
                        .orElse(null);
                if (spawnedTroop.getAttackScope() == Card.AttackScope.AIR_GROUND){
                    nAir++;
                }
            }
